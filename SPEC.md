...
5e. **Real photos only.** Asked for photos, the partner searches Wikimedia Commons (`find_photos`) and pins the matches as photo notes, captioned from each file's own description and credited from its metadata. A photo note must use a file that search returned that turn; nobody is identified from their face. When Commons has nothing that shows the thing itself, it may pin a picture by the authoritative page that publishes it (`photo_page`: a police appeal page, an archive record, a newspaper's single-photo page it read that turn); the server fetches that page's own lead image (og:image) through `/api/page-image` (https only, public hosts only at every redirect, size and time limits, images only), and the file credits the page ("photo as published by …; rights stay with the publisher"). Never an article's generic lead image, a logo or an illustration passed off as evidence.
